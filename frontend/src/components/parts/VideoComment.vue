  ...
  methods: {
    commentList(result) {
      for (let i = 0; i < count; i++) {
        this.comments.push(
          result.data.items[i].snippet.topLevelComment.snippet.textDisplay +
            "\n" +
            "\n"
        );
        ...
        if (
          result.data.items[i].replies.comments.length ==
          result.data.items[i].snippet.totalReplyCount
        ) {
          for (
            let j = 0;
            j < result.data.items[i].replies.comments.length;
            j++
          ) {
            this.threads.push(
              result.data.items[i].replies.comments[j].snippet.textDisplay +
                "\n" +
                "\n"
            );
          }
      ...
      youtube
        ...
        .then((res) => {
          ...
          for (let k = 0; k < res.data.items.length; k++) {
            this.threads.push(
              res.data.items[k].snippet.textDisplay + "\n" + "\n"
            );
          }
        })
        .catch((err) => {
